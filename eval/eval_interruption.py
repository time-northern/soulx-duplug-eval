...
def evaluate_interruption_run(
    ...
    effective_rows = [row for row in rows if row["scenario"] == "interruption"]
    on_time_rows = [
        {
            **row,
            "passed": bool(row.get("within_latency_threshold", False)),
        }
        for row in effective_rows
    ]
    invalid_rows = [row for row in rows if row["scenario"] != "interruption"]
    effective = scenario_language_macro_rate_summary({"interruption": effective_rows})
    on_time = scenario_language_macro_rate_summary({"interruption": on_time_rows})
    ...
    sample_weighted_on_time = rate_summary(on_time_rows)
    ...
    metrics: Dict[str, Any] = {
        ...
        "protocol": {
            ...
            "latency_rule": "first detected nonidle current_time_sec - speech onset",
            "latency_threshold_role": "diagnostic_only",
        },
        "per_category": per_category_summary(rows),
        "aggregates": {
            "effective_intent_recognition_rate": effective,
            "on_time_interruption_rate": on_time,
            "invalid_intent_rejection_rate": invalid,
        },
        "sample_weighted_aggregates": {
            "effective_intent_recognition_rate": sample_weighted_effective,
            "on_time_interruption_rate": sample_weighted_on_time,
        ...
        "technical_targets": {
            "effective_intent_false_rejection_rate": strict_target(
                effective_false_rejection,
                "<",
                float(thresholds["interruption_effective_false_rejection_lt"]),
            ),
            "invalid_intent_rejection_rate": strict_target(
                invalid["rate"],
                ">",
                float(thresholds["interruption_invalid_rejection_gt"]),
            ),
        },
        "not_evaluated": list(config.get("not_evaluated") or []),
    }
    output_dir = run_dir / "evaluation" / "interruption"
    samples_path = output_dir / "samples.jsonl"
    metrics_path = output_dir / "metrics.json"
    report_path = output_dir / "report.md"
    atomic_write_jsonl(samples_path, rows)
    atomic_write_json(metrics_path, metrics)
    atomic_write_text(report_path, render_report("Interruption scene evaluation", metrics))
    update_manifest_evaluation(run_dir, "interruption", metrics_path)
    return metrics
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate interruption-scene metrics from saved state evidence."
    )
...
if __name__ == "__main__":
    ...
    result = evaluate_interruption_run(args.config, args.run_id, args.run_dir)
    print(
        f"Interruption evaluation complete: run={result['run_id']} "
        f"effective={result['aggregates']['effective_intent_recognition_rate']['rate']} "
