#!/usr/bin/env bash
#SBATCH -J soulx_noise_refresh
#SBATCH -p gpu_ai
#SBATCH -n 1
#SBATCH -c 2
#SBATCH -G 1
#SBATCH -o soulx_noise_refresh_%J.out
#SBATCH -e soulx_noise_refresh_%J.err

# Submit from the repository root. Only background-noise inference is replaced;
...
if [[ ! -f "$EVAL_ROOT/infer/rerun_background_noise.py" ]]; then
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: sbatch SoulX-Duplug-Eval/rerun_background_noise.sh --run-id <completed-run-id> [--config <eval-config>]"
      exit 0 ;;
...
echo "protocol=noise_first_20s_plus_2s_silence"

"$PYTHON_BIN" "$EVAL_ROOT/infer/rerun_background_noise.py" \
